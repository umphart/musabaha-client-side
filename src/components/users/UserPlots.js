...
const formatCurrency = (value) => {
 ...
 useEffect(() => {
  const fetchSubscriptions = async () => {
    try {
      const response = await fetch(`https://musabaha-server.onrender.com/api/subscriptions?email=${user.email}`);
...
const handleProceedToPayment = (subscription) => {
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://musabaha-server.onrender.com/api/user-payments', {
        method: 'POST',
        body: formData
      });
      ...
      if (result.success) {
        ...
        const subscriptionsResponse = await fetch(`https://musabaha-server.onrender.com/api/subscriptions?email=${user.email}`);
